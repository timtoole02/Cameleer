...
function read(filePath) {
  ...
  if (!fs.existsSync(fullPath)) {
    console.error(`Kanban receipt smoke failed: missing ${filePath}`);
    process.exit(1);
  }
...
function assertMatch(filePath, regex, message) {
  ...
  if (!regex.test(content)) {
    console.error(`Kanban receipt smoke failed in ${filePath}: ${message}`);
    process.exit(1);
  }
}
...
console.log('Running Kanban receipt smoke test...');
...
assertMatch('src/components/kanban/TaskDetailDrawer.tsx', /disabled=\{busy \|\| Boolean\(completeDisabledReason\)\}/, 'Complete button must be disabled by receipt guard.');
assertMatch('src/components/kanban/TaskDetailDrawer.tsx', /WorkReceiptPanel/, 'Task drawer must render receipt panel.');
...
assertNoMatch('src/pages/KanbanPage.tsx', /invoke\s*\(/, 'Kanban page must not call Tauri invoke directly.');
assertNoMatch('src/components/kanban/TaskDetailDrawer.tsx', /invoke\s*\(/, 'Kanban drawer must not call Tauri invoke directly.');
assertNoMatch('src/components/kanban/WorkReceiptPanel.tsx', /invoke\s*\(/, 'Receipt panel must not call Tauri invoke directly.');
...
console.log('Kanban receipt smoke test passed.');
