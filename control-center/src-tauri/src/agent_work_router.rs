...
pub fn get_agent_work_queue(agent_id: &str, conn: &Connection) -> Result<Vec<KanbanCard>, String> {
    ...
    let h_iter = handoff_stmt.query_map([agent_id], |row| {
        let deps_str: Option<String> = row.get(5)?;
        let deps: Vec<String> = deps_str
            .map(|s| serde_json::from_str(&s).unwrap_or_default())
            .unwrap_or_default();
...
pub fn select_next_work(agent_id: &str, work_queue: &[KanbanCard]) -> Option<KanbanCard> {
    ...
    if let Some(card) = work_queue.iter().find(|c| c.status == "In Progress" && c.blocked_by.is_none()) {
    ...
    if let Some(card) = work_queue.iter().find(|c| c.status == "Review" && c.blocked_by.is_none()) {
        return Some(card.clone());
    }

    // 5. Pull next assigned Ready card
    if let Some(card) = work_queue.iter().find(|c| c.assigned_agent_id.as_deref() == Some(agent_id) && c.status == "Ready" && c.blocked_by.is_none()) {
        return Some(card.clone());
    }
